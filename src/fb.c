...
U8 fb_fadeIn()
{
	...
	while (fade < 8)
	{
		...
		sysvid_setGammaSDL((U8)(56 + 255.0 * (1 - 2.0/(fade+2.0))));
		fade++;
...
U8 fb_fadeOut()
{
	...
	while (fade < 8)
	{
		sysvid_setGammaSDL((U8)(255.0 * 3.0/(fade+3.0)));
		fade++;
		game_rects = &draw_SCREENRECT; // FIXME
		return FALSE;
	}
